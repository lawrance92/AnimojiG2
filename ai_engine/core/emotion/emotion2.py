...
ref_dir = os.listdir('D:/IDE/PythonProject/emojidetect2/ai_engine/data/sad_ref')
for file in ref_dir:
    sad_models.append(file)
    sad_mfcc.append(get_mfcc('D:/IDE/PythonProject/emojidetect2/ai_engine/data/sad_ref/' + file))
...
ref_dir = os.listdir('D:/IDE/PythonProject/emojidetect2/ai_engine/data/fear_ref')
for file in ref_dir:
    fear_models.append(file)
    fear_mfcc.append(get_mfcc('D:/IDE/PythonProject/emojidetect2/ai_engine/data/fear_ref/' + file))
...
def match(tar_path):
    ...
    if tar_path.split('-')[2] == '04':
        ...
        min_dis = dtw(tar_mfcc, sad_mfcc[0])
        for i in range(0, len(sad_mfcc)):
            dis = dtw(tar_mfcc, sad_mfcc[i])
            if dis < min_dis:
                min_dis = dis
                flag = i
        return sad_emot[flag]

    elif tar_path.split('-')[2] == '06':
        tar_mfcc = get_mfcc(tar_path)
        min_dis = dtw(tar_mfcc, fear_mfcc[0])
        for i in range(0, len(fear_mfcc)):
            dis = dtw(tar_mfcc, fear_mfcc[i])
            if dis < min_dis:
                min_dis = dis
                flag = i
        return fear_emot[flag]
